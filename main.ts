...
//% color=#3455db icon="\uf1b9"
//% block="MAKEROBOT"
//% subcategories=["TRACER Junior", "TRACER Senior", "TRACER Expert", "BLITZ Remote", "BLITZ Robot", "BLITZ RC"]
...
namespace MAKEROBOT {
    ...
    // ==========================================
    // BLITZ RC BLOCKS (Cytron MDDRC5 - IND Mode)
    // ==========================================

    /**
     * Initialize pins for Cytron MDDRC5 RC Driver (Ensure board switch is set to IND mode)
     */
    //% blockId=blitz_rc_init
    //% block="init BLITZ RC | Left (RC1) %pinLeft | Right (RC2) %pinRight"
    //% pinLeft.defl=AnalogPin.P1 pinRight.defl=AnalogPin.P2
    //% subcategory="BLITZ RC"
    //% group="Setup"
    //% weight=100
    export function rcInit(pinLeft: AnalogPin, pinRight: AnalogPin): void {
        rcLeftPin = pinLeft;
        rcRightPin = pinRight;
        rcStop(BLITZRCMotor.Both);
    }

    /**
     * Drive an individual motor or both motors independently
     */
    //% blockId=blitz_rc_drive_motor
    //% block="BLITZ RC %motor motor | speed %speed | percent"
    //% speed.min=-100 speed.max=100 speed.defl=50
    //% subcategory="BLITZ RC"
    //% group="Movement"
    //% weight=90
    export function rcDriveMotor(motor: BLITZRCMotor, speed: number): void {
        speed = Math.max(-100, Math.min(100, speed));
        let pulseUs = 1500 + (speed * 5); // Map -100 to 100 -> 1000us to 2000us

        if (motor == BLITZRCMotor.Left || motor == BLITZRCMotor.Both) {
            pins.servoSetPulse(rcLeftPin as number, pulseUs);
        }
        if (motor == BLITZRCMotor.Right || motor == BLITZRCMotor.Both) {
     ...
     */
    //% blockId=blitz_rc_drive_dual
    //% block="BLITZ RC drive | Left speed %leftSpeed | percent | Right speed %rightSpeed | percent"
    ...
    export function rcDriveDual(leftSpeed: number, rightSpeed: number): void {
        rcDriveMotor(BLITZRCMotor.Left, leftSpeed);
        rcDriveMotor(BLITZRCMotor.Right, rightSpeed);
    }

    /**
     * Stop selected RC motor(s) by outputting the neutral 1500us RC pulse
     */
    //% blockId=blitz_rc_stop
    //% block="BLITZ RC stop %motor motor"
    //% subcategory="BLITZ RC"
    //% group="Movement"
    //% weight=80
    export function rcStop(motor: BLITZRCMotor): void {
        if (motor == BLITZRCMotor.Left || motor == BLITZRCMotor.Both) {
